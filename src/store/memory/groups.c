...
static ngx_int_t group_add_channel_internal(nchan_group_t *shm_group, int multi, int self_owned, int n) {
  if(shm_group) {
    if(multi) {
      ngx_debug_atomic_fetch_add((ngx_atomic_uint_t *)&shm_group->multiplexed_channels, n);
    }
    else if (self_owned) {
      ngx_debug_atomic_fetch_add((ngx_atomic_uint_t *)&shm_group->channels, n);
    }
  }
  return NGX_OK;
}
...
static ngx_int_t group_add_message_internal(nchan_group_t *shm_group, size_t mem_sz, size_t file_sz, int n) {
  ...
  if(shm_group) {
    ngx_debug_atomic_fetch_add((ngx_atomic_uint_t *)&shm_group->messages, n);
    ngx_debug_atomic_fetch_add((ngx_atomic_uint_t *)&shm_group->messages_shmem_bytes, n * mem_sz);
    if(file_sz > 0) {
      ngx_debug_atomic_fetch_add((ngx_atomic_uint_t *)&shm_group->messages_file_bytes, n * file_sz);
    }
  }
  return NGX_OK;
}
...
static ngx_int_t group_add_subscribers_internal(nchan_group_t *shm_group, int n) {
  if(shm_group) {
    ngx_debug_atomic_fetch_add((ngx_atomic_uint_t *)&shm_group->subscribers, n);
  }
  return NGX_OK;
}
